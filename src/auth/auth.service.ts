...
export class AuthService {
    ...
    async signUpEmployer(dto:EmployerAuthDto):Promise<any>{
        ...
        const tokens=await this.getTokens(newEmployerLogin.id,newEmployerLogin.email,newEmployerLogin.role)
    ...
    async signUpJobSeeker(dto:JobSeekerAuthDto):Promise<any>{
        ...
        await newJobSeekerLogin.save((err, savedData) => {
            if (err) {
              //console.error(err);
              throw new HttpException("A problem has occured",HttpStatus.BAD_REQUEST);
            } else {
        ...
        await newJobSeeker.save((err, savedData) => {
            if (err) {
              //console.error(err);
              throw new HttpException("A problem has occured",HttpStatus.BAD_REQUEST);
            } else {
        ...
        const tokens=await this.getTokens(newJobSeekerLogin.id,newJobSeekerLogin.email,newJobSeekerLogin.role);
